...
def test_simple_bolt_quote():
    ...
    quotes, errors = gather_quotes(config, lines_from(text))
    assert errors == []
    assert len(quotes) == 1
    q = quotes[0]
...
def test_single_file_source_rejects_id():
    ...
    assert quotes == []
    assert len(errors) == 1
    assert "takes no id" in str(errors[0])
...
def test_dir_source_requires_id():
    ...
    assert quotes == []
    assert len(errors) == 1
    assert "expected '#<id>'" in str(errors[0])
...
def test_without_keep_going_stops_scanning_at_first_syntax_error():
    config = make_config([bolt_source()])
    text = "\n".join([
...
def test_keep_going_collects_all_syntax_errors_and_valid_quotes():
    config = make_config([bolt_source()])
    text = "\n".join([
        "# BOLT #11 no colon here",
        "# BOLT #2: second quote",
    ...
    quotes, errors = gather_quotes(config, lines_from(text), keep_going=True)
    assert [q.id for q in quotes] == [2]
    assert len(errors) == 2
...
def test_draft_commit_excluded():
    config = make_config([bolt_source()])
    text = "# BOLT-abc123def #11: draft text"
    quotes, errors = gather_quotes(config, lines_from(text), include_commit=["other"])
    assert errors == []
    assert quotes == []
